...
class Solution:
    def findWinners(self, matches: list[list[int]]) -> list[list[int | None]]:
        win_num = set()
        lose_num = set()
        lost_nums = set()
        for x in matches:
            win_num.add(x[0])
            if lose_num.__contains__(x[1]):
                lost_nums.add(x[1])
            lose_num.add(x[1])
        res_win = list(win_num-lose_num)
        res_lose = list(lose_num-lost_nums)
        res_win.sort()
        res_lose.sort()

...
class BdoSpider(scrapy.Spider):
    name = "bdo"

    def __init__(self, config_path='config.ini', *args, **kwargs):
        super(BdoSpider, self).__init__(*args, **kwargs)
    ...
    def _load_config(self):
        ...
        for page in range(1, 33):
            self.start_urls.append(f"{start_urls}/%E8%B5%84%E8%AE%AF/%E8%B5%84%E8%AE%AF%E6%9F%A5%E8%AF%A2.aspx?type=event&pagesize=32&page={page}")
        # 处理逗号分隔的域名
    ...
    def parse(self, response: object, depth=1):
        ...
        # 获取完全清理后的文本内容
        if depth == 2:
            insight_content = response.css('#insight-detail ::text').getall()
    ...
    def _is_valid_html_url(self, href):
        ...
        if not href or href.startswith(('mailto:', 'javascript:', 'tel:')):
            return False
        if '/en-gb/资讯' not in href:
            return False
        if "page=" in href and re.findall(r'page=\d+', href)[0] > "32":
            return False
